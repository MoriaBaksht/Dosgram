...
const posts = useSelector(store => store.post.postsList)
  const user = useSelector(store => store.user.user)
const idUser = user.id
// const filteredUser = users.filter(img => img.user.id === idUser)
useEffect(() => {
...
useEffect(() => {
  ...
  return (
   ...
   <div className="one" style={{border:`10px solid ${color}`,marginRight:'90%' ,paddingLeft:'50px',paddingRight:'50px',width:'130%',marginBottom:'-0%'
  }}>
      <br></br>
      ...
      <div >
        <div className="mainDiv"><br />
          <div className="child">
            ...
            navigate('/posts',{state:{name:"food",id:1}})}} ></button>
            <h3 >food</h3>        
          </div>

          <div className="child">         
            <button className="buttonRecipes" onClick={()=>{navigate('/posts',{state:{name:"recipes"}})}}></button>
            <h3 >recipes</h3>
          </div>
          ...
          <div className="child">         
            <button  className="buttonShops" onClick={()=>{navigate('/posts',{state:{name:"shops",id:2}})}}></button>
            <h3 >shop</h3>
          </div>

          <div className="child">        
            <button className="buttonLand" onClick={()=>{navigate('/posts',{state:{name:"landSpace"}})}} ></button>
            <h3 >landScapes</h3>
          </div>

          <div className="child">         
            <button className="buttonTrip" onClick={()=>{navigate('/posts',{state:{name:"trips"}})}}></button>
            <h3 >trips</h3>
          </div>

          <div className="child">         
            <button  className="buttonMakeUp" onClick={()=>{navigate('/posts',{state:{name:"makeUp"}})}}></button>
            <h3 >make-up</h3>
          </div>
        </div>
